...
class TestingConfig(object):
    def __init__(self):
        self.wlast = '/mnt/lustre/qiudi/relight_tf/Experiment_taxo/training_2019-12-05-18-56_relight_taxo_nohl/models/-300000'
...
class RelightLearner(object):
    def __init__(self, dataset, is_training):
        ...
        if is_training:
            ...
            self._declare_placeholders()
            self._build_loader(self.testing_config.path, 'test')
    ...
    def _build_graph(self):
       ...
       # RGB encoder
            rgb_input = tf.concat([self.rgb_src, self.dist_to_cam], 3)
        ...
        # normal decoder
            ...
            self.loss_albe = L1loss(self.albe_pred * self.conf, self.albedo * self.conf) \
                + sobel_gradient_loss(self.albe_pred*self.conf, self.albedo * self.conf) * 5 

            self.loss_render =  L1loss(self.rendered_pred * self.conf, self.rendered * self.conf)

            # self.loss_hl = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=hl_pred * self.conf, labels=self.hl * self.conf))
    ...
    def _build_summary(self):
        ...
        rgb_src = tf.summary.image('rgb_src', self.rgb_input[0, None, ...])
        ...
        # loss_hl = tf.summary.scalar('loss_hl', self.loss_hl)
            ...
            if self.idxtype == 'random':
                ...
                self.sess.run([self.enqueue_op_mat],
                            feed_dict={
                                ...
                                self.dist_to_cam_img2 : data_idx2['dist_to_cam'],
                                self.roughness_img2 : data_idx2['roughness'],
                                self.point_cloud_img2 : data_idx2['point_cloud'],
                ...
                self.sess.run([self.enqueue_op_mat],
                            feed_dict={
                                ...
                                self.dist_to_cam_img2 : data_idx['dist_to_cam'],
                                self.point_cloud_img2 : data_idx['point_cloud'],
    ...
    def test(self, step=0):
        ...
        while view < dataset_len:
            ...
            normal_error = self._compute_l2(nl_pred, nl, conf)
            ...
            loss_normal[view:view+self.bs] += normal_error
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_training', type=int, default=0, help='True or False')
    ...
    if args.is_training:
        ...
        args.save = args.save + 'testing_' + file_name
    makedirs(args.save)
    os.chdir(args.save)
